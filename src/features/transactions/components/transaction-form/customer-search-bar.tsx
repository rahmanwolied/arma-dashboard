...
export default function CustomerSearchField({
	...
	const filteredCustomers = existingCustomers?.customers?.filter(
		(customer) =>
			customer.name.toLowerCase().includes(searchValue.toLowerCase()) ||
			customer.primaryPhone.toLowerCase().includes(searchValue.toLowerCase()),
	);
	...
	const handleSelectCustomer = (
		...
		const customerValue: CustomerValue = {
			...
			address: customer.addresses[0].addressLine,
			isNew: false,
		};
	...
	const handleInputChange = (search: string) => {
		...
		if (exactMatch) {
			const customerValue: CustomerValue = {
				...
				phone: exactMatch.primaryPhone,
				address: exactMatch.addresses[0].addressLine,
				isNew: false,
			};
			onChange?.(customerValue);
		} else {
